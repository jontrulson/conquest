 ...
 * Initial revision
 *
 * Revision 1.18  1996/12/07  04:06:47  jon
 ...
 *   unfortunate. ;-\
 *
 * Revision 1.17  1996/11/23  06:41:27  jon
 * - replaced PLANET_SHITFACE with PLANET_ALTAIR, a new, non-core planet, yet
 *   *quite* valuable stratigically. ;-)
 * - modified to display messages in cyan.
 * - mods to support extra msg line (line 25)
 *
 * Revision 1.16  1996/07/03  02:15:32  jon
 * - added new config option, rcv_player_msgs option.  See ~/.conquestrc
 *   more more info.
 *
 * Revision 1.15  1996/07/02  20:37:19  jon
 * - removed null char at end of file. cc doesn't like that.
 *
 * Revision 1.14  1996/07/02  20:06:11  jon
 * - code cleanup
 * - changed hell to be SELF_RULED instead of EMPIRE.  There is no
 *   Empire. yet.
 * - randomized 'virtual armies' for suns.  Used to determine
 *   damage a ship takes when passing near/over one
 * - added function cmpplanet() for use by qsort().
 * - modified sortplanets() to use qsort to sort the planet
 *   vector to be used as a lookup table when displaying a
 *   planet list.
 *
 * Revision 1.13  1996/05/25  00:42:59  jon
 * *** empty log message ***
 *
 * Revision 1.12  1996/04/28  06:04:09  jon
 * - fixed the orbital velocity of Syrinx.  was waaay too slow.
 * - added conf_NoRobotMsgs check to readmsg().  if TRUE you won't
 *   get msgs back from friendly robots.
 * - modified readmsg to return an int - TRUE if a message was actually
 *   display, FALSE otherwise (if conf_NoRobotMsgs is TRUE for instance)
 *
 * Revision 1.11  1996/03/31  23:54:06  jon
 * - some formatting changes, and removel of useless comments.
 * - changed an erroneous cerror() call to a clog() call.
 * - changed sendmsg() to allow a [RETURN] to act like [TAB], ie:
 *   send message to last person I sent a msg to.
 *
 * Revision 1.10  1995/02/08  05:03:33  jon
 * global change of all _And_'s to &&'s and _Or_ to ||'s.  other minor
 * 'corrections' and cleanups.
 *
 * Revision 1.9  1995/02/02  05:20:43  jon
 * initship() - fixed ulastentry indexing bug, replaced scopy with strcpy.
 *
 * Revision 1.8  1995/01/30  06:06:25  jon
 ...
 *
 * Revision 1.5  1995/01/18  05:08:51  jon
 * histlist():, loghist(): fixed some bugs... no more trashinbg of entries,
 * and histlist() lists hists correctly :-)
 *
