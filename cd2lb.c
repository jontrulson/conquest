 ...
 *
 * $Log$
 * Revision 1.1  1996/12/08 05:06:34  jon
 * Initial revision
 *
 * Revision 1.15  1996/12/08  05:05:50  jon
 * - added extra ncurses check for bkgdset() call
 *
 * Revision 1.14  1996/11/30  20:12:38  jon
 * - removed seperate fd/newterm() curses code.  It didn't work
 *   properly with anything other than SysV4 curses and
 *   ncurses 1.9.9e.  Standard initscr() is used now.  This should make
 *   conquest somewhat more likely to work on other reasonably
 *   modern curses implementations.
 * - remove qiflush() call from cdinit().  It wasn't really required,
 *   and wasn't supported on anything other than sys4/ncurses 1.9.9e.
 *
 * Revision 1.13  1996/11/23  06:13:28  jon
 * - added conf_DoLimitBell mod.  see .conquestrc
 * - modified cdinit() to not use bkgdset(...) if running under ncurses.  It
 *   has a bug that corrupts the colors if a backgroud is set, but not at
 *   color pair 0 (which under SVID is illegal). Bummer.  if using ncurses,
 *   you might prefer a black background, so if running under an xterm,
 *   set your background to black.
 * - modofied cdinit() to allow lines > 25.  If you have lines > 25, then
 *   the 25th line will be used to display messages instead of overwriting
 *   what ever is displayed on lines 23-24, like an (i)nfo command.
 * - autoconf mods.
 *
 * Revision 1.12  1996/11/23  06:11:25  jon
 * - removed calls to erase(), it was to bandwidth intensive.  Use clear()
 *   intead.
 * - optimized cdclra() so that clrtoeol() would be used when the last
 *   col in the rect was equal to maxcol.
 *
 * Revision 1.11  1996/07/02  03:00:31  jon
 * - removed old/unness macros
 * - removed USE_CURSESIO stuff.  always do that now.
 * - mods to support linux (ncurses)
 * - removed USETIMER stuff.  you always USETIMER now.
 * - added check to cdinit to make sure Cols >= 80 &&
 *   Lins >= 24
 *
 * Revision 1.10  1996/04/28  05:36:22  jon
 * - removed LOTS of dead code, unneccessary comments, #define's, etc...
 * - added conf_NoColor check to cdinit()
 *
 * Revision 1.9  1996/03/31  23:35:02  jon
 * - fixed delete bug in cdgetp().
 * - added iBufInit() call to cdinit().  executed if ENABLE_MACROS is defined.
 *
 * Revision 1.8  1996/03/16  21:48:21  jon
 * cerror() -> clog() chgs..
 * added notimeout() curses call in another attempt to solve some
 * timing problems, and delays when reading the function keys.  I seems
 * to work, and has removed many unexplained 1 seconds delays when
 * doing certain things.  And even with the timer disabled, function
 * key still seems to work, and without a 1 second delay after typing
 * one.
 *
 * Revision 1.7  1995/05/26  03:28:48  jon
 * Much clean up and removal of uneeded code.  Fixed bug that would
 * cause core dump if TERM not set properly.
 *
 * Revision 1.6  1995/02/08  05:03:33  jon
 * global change of all _And_'s to &&'s and _Or_ to ||'s.  other minor
 * 'corrections' and cleanups.
 *
 * Revision 1.5  1995/01/30  06:06:25  jon
 * Added signal handling capability.  Now Gods can INTR out nicely,
 * others can't.  many small bug fixes.
 *
 * Revision 1.4  1995/01/27  04:37:40  jon
