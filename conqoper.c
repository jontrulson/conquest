 ...
 *
 * $Log$
 * Revision 1.1  1997/03/28 22:53:58  jon
 * Initial revision
 *
 * Revision 1.21  1996/12/07  04:06:47  jon
 ...
 *   unfortunate. ;-\
 *
 * Revision 1.20  1996/11/30  20:20:41  jon
 * - removed cdclear() call from player edit screen to remove flicker.
 *
 * Revision 1.19  1996/11/23  06:45:18  jon
 * - mods to support the -C (create new sys-wide conquestrc file) option.
 * - mods to support sysconf_DoRandomRobotKills
 * - many formating bugs fixed
 * - mods to player edit so that arrow keys/keypad can be used for nav.
 * - mods to support extra msg line.
 *
 * Revision 1.18  1996/07/12  19:45:13  jon
 * - fixed oppedit so that (T)eam ownership works, and in the
 *   right circumstances.
 *
 * Revision 1.17  1996/07/06  01:44:56  jon
 * - added OPER_MSG_BEEP #definable to defs.h.  if defined, beep when
 *   conqoper reads a message.
 *
 * Revision 1.16  1996/07/02  20:11:34  jon
 * - code cleanup
 * - changes to launch() calls
 * - modified debugdisplay to show torp status in symbolic form
 *   rather than numeric.
 * - added qsort() call to sort the debugging planet list also.
 * - removed beep that occured when conqoper read a message
 *   (which is all messages).  This was a little annoying.
 * - fixed LF reference in planet edit.  Is now TAB.
 * - converted while{} into a do{} in planet list.  This will
 *   remove the 1 second delay before the planets are listed.
 * - modified debugdisplay to show the process pid in dec format
 *   rather than hex (which *would* be normal for VMS ;-)
 *
 * Revision 1.15  1996/05/25  00:44:54  jon
 * - added GetSysConf() call
 *
 * Revision 1.14  1996/05/02  01:12:39  jon
 * - added call to GetSems() if USE_SEMS is defined
 * - changed lock status display to relect semaphore counts if
 *   USE_SEMS is defined
 *
 * Revision 1.13  1996/04/28  18:04:10  jon
 * - fixed torp formatting bug in debugdisplay()
 *
 * Revision 1.12  1996/04/28  06:06:00  jon
 * - fixed some formatting problems in the debugging watch()
 * - removed VERSION_NUMBER and VERSION_DATE - handled by version.c now
 * - removed dead code
 *
 * Revision 1.11  1996/03/31  23:56:47  jon
 * - added GetConquestUID() and GetConquestGID() calls.
 * - added priority code.  if SET_PRIORITY is defined (in defs.h) then
 *   conqoper will run at CONQUEST_PRI priority.
 *
 * Revision 1.10  1996/03/16  21:56:08  jon
 * cerror() -> clog() changes.
 *
 * Revision 1.9  1995/05/26  03:25:13  jon
 * Fixed up message reading logic to notify of incoming msgs
 * with a beep.
 *
 * Revision 1.8  1995/02/08  06:57:41  jon
 ...
 *
 * Revision 1.6  1995/01/29  05:57:17  jon
 * oppedit() - fixed planet formatting bug.
 *
 * Revision 1.5  1995/01/27  04:37:40  jon
 ...
 * rmoval of NOLOCKON, other fixes...
 *
